...
const SUB_FORMS = [
  {
    key: 'infrastructure',
    slug: 'infrastructure-classroom-status',
    label: 'Infrastructure Status',
    description: 'Campus status, building type, room counts, washrooms, playground, garden and renovation',
    icon: Building2,
    color: 'from-emerald-500 to-emerald-600',
    bg: 'bg-emerald-50',
    text: 'text-emerald-600',
    trackKey: 'infrastructure', // maps to dashboard API form key
  },
  {
    key: 'classroom-status',
    slug: 'classroom-status',
    label: 'Classroom Status',
    description: 'Digital classrooms, floor sitting, whiteboard/blackboard counts and furniture details',
    icon: Columns3,
...
export default function InfrastructureSubPage() {
  const { id } = useParams();
  ...
  if (loading) {
    return (
      <>
        <Header title="Infrastructure & Classroom" />
  ...
  return (
    <>
      <Header
        title="Infrastructure & Classroom"
          ...
          <CardContent className="p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900">Infrastructure & Classroom Forms</h2>
                  <p className="text-sm text-gray-500">{school.name} &bull; <span className="font-mono text-xs text-gray-400">{school.code}</span></p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="rounded-xl bg-gray-50 px-4 py-2 text-center">
                  <p className={`text-2xl font-bold ${allDone ? 'text-green-600' : 'text-emerald-600'}`}>{doneCount}</p>
            ...
            {allDone && (
              <div className="mt-4 flex items-center gap-2 rounded-xl bg-green-50 px-4 py-3 border border-green-200">
                ...
                <p className="text-sm font-medium text-green-700">All {SUB_FORMS.length} forms have been submitted!</p>
              </div>
            )}
          </CardContent>
        </Card>
