...
import type { FieldType, Survey, LinkedEntityType, SurveyCategory } from '@/types';
...
export default function EditSurveyPage() {
    ...
    const fetchSurvey = async () => {
      try {
        ...
        setSurvey({
          ...
          createsSchoolRecord: data.createsSchoolRecord || false,
        });
          ...
          setSections(
            data.sections.map((s) => ({
              ...
              fields: (s.fields || []).map((f) => ({
                ...
                maxFileSize: f.maxFileSize || 5242880,
              })),
            })),
          );
        } else {
          // Legacy: fields without sections
          setSections([
            {
              title: 'Section 1',
              description: '',
  ...
  const handleSave = async (status: 'draft' | 'published' = 'draft') => {
    ...
    if (status === 'published') {
      setConfirmModal({
        ...
        message:
          'Once published, survey fields cannot be edited. Are you sure?',
  ...
  const doSave = async (status: 'draft' | 'published') => {
    try {
      ...
      await api.patch(`/surveys/${id}`, payload);
      router.push(`/surveys/${id}`);
    } catch (error: any) {
      alert(error.response?.data?.message || 'Failed to save survey');
    } finally {
      setSaving(false);
    }
  };
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  >
                    ...
                    {survey.category && !categories.find(c => c.name === survey.category) && (
