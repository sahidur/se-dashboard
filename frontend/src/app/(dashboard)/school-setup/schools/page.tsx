...
export default function SchoolSetupPage() {
  ...
  const toggleStatus = async (s: DcSchool) => {
    try {
      await api.patch(`/data-collection/schools/${s.id}`, { isActive: !s.isActive });
      queryClient.invalidateQueries({ queryKey: ['dc-schools'] });
      queryClient.invalidateQueries({ queryKey: ['dc-schools-minimal'] });
    } catch (e) {
      alert(getErrorMessage(e, 'Error updating school status'));
    }
  };
  ...
  return (
    <>
      <Header
        title="Create School"
        subtitle="Create and manage schools — students, classes and fees link to these schools"
        actions={
          <Button
            onClick={() => {
              setEditSchool(null);
      ...
      <div className="p-4 sm:p-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 py-20">
            ...
            <p className="mb-4 text-sm text-gray-400">Create a school to start adding students and fees.</p>
            <Button
              onClick={() => {
                setEditSchool(null);
                setModalOpen(true);
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-50/50">
                    ...
                    <th className="hidden px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 xl:table-cell">Teachers / Students</th>
                ...
                <tbody>
                  {paged.length === 0 ? (
                    ...
                    paged.map((s, idx) => (
                      <tr key={s.id} className="border-b border-gray-50 transition-colors hover:bg-brand-50/30 even:bg-gray-50/30">
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400">
                                {s.code || '-'} · {categoryLabel(s.schoolCategory)}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="hidden px-4 py-3 text-sm text-gray-500 xl:table-cell">
                          {s.totalTeachers ?? '-'} / {s.totalStudents ?? '-'}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              title={s.isActive === false ? 'Set Active' : 'Set Inactive'}
                            >
                              <Power size={14} />
                            </button>
                          </div>
